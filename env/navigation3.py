...
OBSTACLE = [[[-6, -4], [-1.5, 1.5], [0, 4.0]]]

CAUTION_ZONE = [[[-32/5, -18/5], [-12/5, 12/5], [-2/5, 6/5]]]
...
class Navigation3(Env, utils.EzPickle):
    def __init__(self):
        ...
        self.action_space = Box(-np.ones(3) * MAX_FORCE,
                                np.ones(3) * MAX_FORCE)
        self.observation_space = Box(-np.ones(3) * np.float('inf'),
                                     np.ones(3) * np.float('inf'))
...
def get_offline_data(num_transitions, task_demos=False, save_rollouts=False):
    ...
    for i in range(num_transitions // 10 // 3):
        rollouts.append([])
        state = np.array(
            [np.random.uniform(-40/5, 10/5),
             np.random.uniform(-25/5, 25/5), np.random.uniform(0, 4)])
        while env.obstacle(state):
            state = np.array(
                [np.random.uniform(-40/5, 10/5),
                 np.random.uniform(-25/5, 25/5), np.random.uniform(0, 4)])
        for j in range(10):
            action = MAX_FORCE*np.clip(np.random.randn(3), -1, 1)
    ...
    for i in range(num_transitions // 10 * 1 // 4):
        rollouts.append([])
        state = np.array(
            [np.random.uniform(-35/5, -30/5),
             np.random.uniform(-12/5, 12/5),np.random.uniform(0, 4)])
        for j in range(10):
            action = MAX_FORCE*np.clip(
                np.array([np.random.uniform(0.5, 1, 1),
                          np.random.randn(1),np.random.randn(1)]), -1, 1).ravel()
            next_state = env._next_state(state, action, override=True)
            constraint = env.obstacle(next_state)
            reward = env.step_cost(state, action)
            transitions.append(
    ...
    for i in range(num_transitions // 10 * 1 // 4):
        rollouts.append([])
        state = np.array(
            [np.random.uniform(-30/5, -20/5),
             np.random.uniform(10/5, 15/5),np.random.uniform(0, 4)])
        for j in range(10):
            action = MAX_FORCE*np.clip(
                np.array([np.random.randn(1),
                          np.random.uniform(-1, -0.5, 1),np.random.randn(1)]), -1, 1).ravel()
            next_state = env._next_state(state, action, override=True)
            constraint = env.obstacle(next_state)
            reward = env.step_cost(state, action)
            transitions.append(
    ...
    for i in range(num_transitions // 10 * 1 // 4):
        ...
        state = np.array(
            [np.random.uniform(-30/5, -20/5),
             np.random.uniform(-15/5, -10/5),np.random.uniform(0, 4)])
        for j in range(10):
            action = MAX_FORCE*np.clip(
                np.array([np.random.randn(1),
                          np.random.uniform(0.5, 1, 1),np.random.randn(1)]), -1, 1).ravel()
            next_state = env._next_state(state, action, override=True)
            constraint = env.obstacle(next_state)
            reward = env.step_cost(state, action)
            transitions.append(
