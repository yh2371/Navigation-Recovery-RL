...
class QRiskWrapper:
    ...
    def update_parameters(self,
        ...
        state_batch = torch.FloatTensor(state_batch).to(self.device).to(torch.float32)
        next_state_batch = torch.FloatTensor(next_state_batch).to(self.device).to(torch.float32)
        action_batch = torch.FloatTensor(action_batch).to(self.device).to(torch.float32)
        mask_batch = torch.FloatTensor(mask_batch).to(self.device).unsqueeze(1).to(torch.float32)
        constraint_batch = torch.FloatTensor(constraint_batch).to(self.device).unsqueeze(1).to(torch.float32)
